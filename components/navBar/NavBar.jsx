...
const NavBar = () => {
    ...
    return (
        <div className='bg-blue-500 h-[13vh] flex justify-between items-center sticky top-0 z-50'>
            <div className='flex items-center text-white text-xl gap-10 h-[12vh] justify-around'>
                ...
                <Link className={`inline-block mt-7 hover:underline ${router.asPath === "/team" ? `underline` : ``}`} href={"/team"}>Team</Link>
            ...
            <div className='w-[10%]'>
                {auth.token ? <span className='bg-white mt-7 text-blue-500 text-[2vh] h-[35px] w-[55%] flex items-center justify-center font-semibold cursor-pointer' onClick={() => { auth.logout() }}>Log out</span>
                    : <Link className='bg-white mt-7 text-blue-500 text-[2vh] h-[35px] w-[55%] flex items-center justify-center font-semibold' href={"login"}>Log in</Link>}
            </div>
        </div>
    )
}
