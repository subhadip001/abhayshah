...
const NavBar = () => {
    ...
    return (
        <div className='bg-blue-500 h-[12vh] flex justify-between items-center sticky top-0 z-50'>
            <div className='flex items-center text-white text-xl w-[50%] h-[12vh] justify-around'>
                ...
                {auth.username !== null ? <Link className={`inline-block ${router.asPath.includes("/dashboard") ? `underline` : ``}`} href={"/dashboard"}>Dashboard ({auth.username})</Link> : <></>}
            </div>
            <div className='w-[10%]'>
